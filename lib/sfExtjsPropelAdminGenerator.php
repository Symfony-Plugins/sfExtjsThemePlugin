...
class sfExtjsPropelAdminGenerator extends sfAdminCustomGenerator
{
  ...
  public function createPartialFile($partialName,$contents='')
  {
    ...
    if (!file_exists($partial))
    {
      ...
      if(is_writable(dirname($partial))) file_put_contents($partial,$contents);
    }
  }
  ...
  function getAjaxButtonToToolbarAction($actionName, $params, $pk_link = false)
  {
    ...
    if ($actionName[0] == '_')
    {
     ...
     ";
        ...
        case 'create':
          ...
          $default_handler_function = "window.location = '".$this->controller->genUrl($this->getModuleName().'/create')."'";
        ...
        case 'refresh':
          ...
          $default_handler_function = "this.store.reload();";
          break;
        case 'print':
          $default_icon = 'printer';
          $default_handler_function = "window.open('".$this->controller->genUrl($this->getModuleName().'/listPrint')."')";
          break;
        case 'pdf':
    ...
    else
    {
      ...
      if ($gridListCreateLink = sfConfig::get('app_sf_extjs_theme_plugin_list_action_handler', null))
      $default_handler_function = $gridListCreateLink."('$handler_url', '".$default_name."', '".$name."');";
    }
    ...
    function getEditAjaxActionToButton($actionName, $params, $edit_ns, $pk_link = false )
    {
      ...
      /**
       * Returns an array of field-names and their type
       *
       * @param string $for
       * @return hierarchically arranged (recursively per related table) array of columns
       */
      //  public function getFieldProperties($for = 'list')
      //  {
      //    $fields = array();
      //
      //    $groupedColumns = $this->getColumnsGrouped($for.".display");
      //    $columns = $this->getListColumns($groupedColumns, true);
      //    $pkn = $groupedColumns['pk']->getName();
      //
      //    // Always add the primary key
      //    $fields[$pkn] = $this->getFieldType($groupedColumns['pk']);
      //
      //    foreach ($columns as $column)
      //    {
      ...
      //      {
      //        $fields[$fieldName] = $this->getFieldType($column);
      //      }
      //    }
      //
      //    return $fields;
      ...
      /**
       * Returns HTML code for a column in ajax mode.
       *
       ...
       * @return string HTML code
       */
      //  public function getColumnAjaxTag($column) //, $params = array(), $for = 'list'
      //  {
      ...
      //
      //      $format = isset($params['date_format']) ? $params['date_format'] : 'MM/dd/yyyy';
      ...
      //      $columnGetter = $this->getColumnGetter($column, true);
      //
      //      return $for == 'edit' ? "$columnGetter ? 'true' : 'false'" : "$columnGetter ? 1 : 0";
      //    }
      //
      ...
      //  {
      //    $result = parent::getColumnListTag($column, $params);
      //    return $result;
      //  }
      ...
      public function getColumnAjaxListDefinition($column, $groupedColumns = array())
      {
        ...
        if(isset($definition['renderer']))
        {
          ...
          {
            //$definition['renderer'] = ''.$definition['renderer']; //TODO add this to it by default, but to early for that now, related renderers and
