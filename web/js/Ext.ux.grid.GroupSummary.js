...
Ext.ux.grid.GroupSummary = function(config){
  Ext.apply(this, config);
};

Ext.extend(Ext.ux.grid.GroupSummary, Ext.util.Observable, {
  init : function(grid){
    this.grid = grid;
...
Ext.ux.grid.GroupSummary.Calculations = {
  ...
  'count' : function(v, record, field, data){
    //only add 1 if field contains data
    if (record.data[field] != '') {
      return data[field+'count'] ? ++data[field+'count'] : (data[field+'count'] = 1);
    } else {
      return data[field+'count'] ? data[field+'count'] : (data[field+'count'] = 0);
    }
  },
