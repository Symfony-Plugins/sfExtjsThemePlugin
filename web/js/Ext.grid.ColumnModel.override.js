...
Ext.grid.ColumnModel.override({
     ...
     * also accepting xtypes: http://extjs.com/forum/showthread.php?p=180237
    ...
    setConfig : function(config, initial){
        ...
        for(var i = 0, len = config.length; i < len; i++){
            if(config[i].xtype){ // create column-objects
            ...
            if (c.editor) {
                ...
                if (!c.editor.field) {
                    ...
                    if (!c.editor.isFormField) {
                        c.editor = Ext.ComponentMgr.create(c.editor, 'textfield'); // get instance from xtype config, default to textfield
                    }
                    // create gridEditor
