...
Ext.ux.grid.RowActions = function(config)
{
  ...
  this.addEvents(
                 /**
                   * @event beforeaction Fires before action event. Return false
                   *        to cancel the subsequent action event.
                   * @param {Ext.grid.GridPanel}
                   *          grid
                   * @param {Ext.data.Record}
                   *          record Record corresponding to row clicked
                   * @param {String}
                   *          action Identifies the action icon clicked. Equals
                   *          to icon css class name.
                   * @param {Integer}
                   *          rowIndex Index of clicked grid row
                   * @param {Integer}
                   *          colIndex Index of clicked grid column that
                   *          contains all action icons
                   */
                 'beforeaction',
                 /**
                 ...
                 'action',
                 /**
                   * @event beforegroupaction Fires before group action event.
                   *        Return false to cancel the subsequent groupaction
                   *        event.
                   * @param {Ext.grid.GridPanel}
                   *          grid
                   * @param {Array}
                 ...
                 'beforegroupaction',
                 /**
                   * @event groupaction Fires when icon in a group header is
                   *        clicked
                   * @param {Ext.grid.GridPanel}
                   *          grid
                   * @param {Array}
...
Ext.extend(Ext.ux.grid.RowActions, Ext.util.Observable, {
   ...
   *      following configuration options of action are recognized: -
   ...
   * @cfg {String} iconIndex Optional, however either iconIndex or iconCls must
   *      be configured. Field name of the field of the grid store record that
   ...
   * @cfg (string} hideIndex Optional. Field name of the field of the grid store
   *      record that contains hide flag (falsie [null, '', 0, false, undefined]
   *      to show, anything else to hide). -
   * @cfg {String} qtipIndex Optional. Field name of the field of the grid store
   *      record that contains tooltip text. If configured, the tooltip texts
   *      are taken from the store. -
   * @cfg {String} tooltip Optional. Tooltip text to use as icon tooltip. It is
   *      ignored if qtipIndex is configured. Use this if you want static
   ...
   * @cfg {String} text Optional. Text to display on the right side of the icon.
   *      Use this if you want static text that are not taken from record.
   *      Ignored if textIndex is set. -
   ...
   *      same way as actions. There is one more action config recognized: -
   * @cfg {String} align Set it to 'left' to place action icon next to the group
   *      header text. (defaults to undefined = icons are placed at the right
   *      side of the group header.
   */
  ...
  onClick : function(e, target)
  {
    ...
    if (false !== row && false !== col && false !== action)
    {
      ...
      if (true !== this.eventsSuspended
        && false === this.fireEvent('beforeaction', this.grid, record, action, row.rowIndex, col))
      {
        return;
      }
    ...
    if (t)
    {
      ...
      // fire events
      if (true !== this.eventsSuspended
        && false === this.fireEvent('beforegroupaction', this.grid, records, action, groupId))
      {
        return false;
      }
