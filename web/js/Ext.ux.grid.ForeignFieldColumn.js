...
// set up foreign-key-column with combobox, dynamic+static comob-store, gridfield-renderer and preloading objects from grid-store
Ext.namespace("Ext.ux.renderer");
Ext.ux.renderer.ComboRenderer = function(options) {
  var value = options.value;
  var combo = options.combo;

  if (value === '') return;  // skip if no value (foreign-key) defined

  var returnValue = 'Ext.ux.renderer problem'; //initial value, replaced below (if everything goes well)
  ...
  // if combos-store does not contain the display-value, try grid-store preloaded field
...
Ext.extend(Ext.ux.grid.ForeignFieldColumn, Ext.util.Observable, {
    
  renderer : function(combo) {
    // in some cases this is required, especially if the displayField is an aggregate function and not a column
    //TODO: KRavEN I do not use editorStore anymore, moved the store to the comboboxAutoLoad...
    // I think this does the same (only I cannot test it for you)
    if(this.editor.store!=undefined && combo.preLoad){
      combo.pageSize = 0;
      combo.editable = false;
      combo.mode = 'local';
      this.editor.store.load(); //don't forget point 3 I mentioned, when having a lot of stores, load will introduce latency... maybe you want to preload all stores in one go? (E.G. fill them with an array instead of json, don't use an URL and remove this load() method)
    }
