...
Ext.ux.ValidationStatus = Ext.extend(Ext.Component, {
  ...
  validTextCls : 'x-status-validtext',

  errorTextCls : 'x-status-errortext',
  ...
  init : function(sb)
  {
    sb.on('render', function()
    {
      ...
      if (this.form)
      {
        ...
        // Have to give the status bar time to render since it happens in
        // afterRender
        (function()
        {
          sb.statusEl.on('click', this.onStatusClick, this, {
            buffer : 200
          });
        }).defer(200, this);
  ...
  onFieldValidation : function(f, msg)
  {
    ...
    if (this.errors.getCount() > 0)
    {
      if (this.statusBar.getText() != this.showText)
      {
        this.statusBar.setStatus({
          text : this.showText,
          iconCls : this.errorTextCls + ' ' + this.errorIconCls
        });
      }
    }
    else
    {
      this.statusBar.setStatus({
        text : this.validText,
        iconCls : this.validTextCls + ' ' + this.validIconCls
      });
    }
  },
  ...
  updateErrorList : function()
  {
    if (this.errors.getCount() > 0)
    {
      ...
      {
        msg +=
          ('<li id="x-err-' + err.field.id + '"><a href="#">' + err.field.fieldLabel + ': ' + err.msg + '</a></li>');
      }, this);
